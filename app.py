...
# NEW: horizontal navbar component
# pip install streamlit-option-menu
from streamlit_option_menu import option_menu  # docs: https://pypi.org/project/streamlit-option-menu/
...
selected = option_menu(
    None,  # no title; we use the bar alone
    list(pages.keys()),
    icons=[
        "speedometer", "bar-chart-line", "telephone", "list-check",
        "people", "currency-dollar", "globe"
    ],  # Bootstrap icons (used by option-menu)
    ...
    styles={
        ...
        "nav-link": {
            ...
            "color": "#334155", "--hover-color": "#eff6ff"
        },
...
# Keep sidebar only for status and filters (navigation removed from sidebar)
...
if current_page == "executive_summary":
    ...
    if 'optimal_calling_windows' in success_pred:
        ...
        fig = px.bar(
            windows_df, x='time', y='success_rate',
    ...
    st.markdown(
        ...
        f'<p><strong>Revenue Confidence Range:</strong> ${confidence_interval/1000:.0f}K - ${confidence_interval[4]/1000:.0f}K</p>'
...
# Footer with AI Model Info (kept)
st.markdown("---")
col1, col2, col3 = st.columns(3)
with col1:
    if 'meta_insights' in ai_insights:
